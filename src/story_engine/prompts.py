...
def get_narrator_style_guide(style: str) -> str:
    """获取风格描述"""
    return NARRATOR_STYLES.get(style, NARRATOR_STYLES["林汉达"])


# ─── 故事生成系统提示 ─────────────────────────
STORY_SYSTEM_PROMPT = """你是一位精通中国古典历史与文学的连环画故事编剧。你的任务是为中国传统白描连环画生成故事内容。

画面风格要求：黑白传统中国老版连环画，{对应画家完整风格描述}，参考图：[画家姓名_序号]，白描单线，传统毛笔线条，上美60年代三国连环画质感，无现代光影，古典古装人物，叙事构图，宣纸素描质感，干净墨线，无彩色，细腻排线。
...
```json
{
  ...
  "era": "历史时期",
  "scene_plan": {
    "foreground": "前景中最突出的人物/物体及其状态",
    "middle_ground": "中景环境，人物的活动区域",
    "background": "背景环境、建筑、山水、天空",
    "character_positions": "画面中各个人物的空间位置关系",
    "actions": "主要人物的动作",
    "camera": "镜头视角（平视/俯视/仰视/特写/中景/远景）",
    "composition": "构图方式（如中心构图、斜线构图，适合竖幅连环画）",
    "lighting": "光线环境（自然光/火光/烛光，用明暗对比增强墨线表现力）"
  },
  "scene_description": "画面场景的详细描述（200-300字），基于scene_plan展开",
...
## 时代约束
人物服饰、建筑、兵器、礼仪必须严格匹配所选时代：
- 先秦/春秋战国：交领右衽深衣、青铜兵器、战车
- 汉代/三国：交领曲裾深衣、冠帻、环首刀、汉戟
- 隋唐：圆领袍、幞头、明光铠、斗拱建筑
- 宋：圆领官袍、展脚幞头、素面铠甲
- 明：补服、乌纱帽、棉甲
- 清：马褂、金钱鼠尾辫、顶戴花翎
